...
/*
 * if enabled , blocks incoming connections when active connections count >= backlog_depth
 * if disabled then newly incoming connections will cause the tcpip thread to abort the oldest
 * connection in the queue to accept new connections only if active connections exceeds
 * CONFIG_LWIP_MAX_ACTIVE_TCP.
 **/
#ifndef TCP_DELAYED_BACKLOG
#define TCP_DELAYED_BACKLOG 1
#endif

#ifndef DEBUG_ESP_ASYNC_TCP
...
typedef void(*AcAckHandler)(void*, AsyncClient*, size_t len, uint32_t time);
typedef void(*AcErrorHandler)(void*, AsyncClient*, err_t error);
typedef void(*AcDataHandler)(void*, AsyncClient*, void *data, size_t len);
typedef void(*AcPacketHandler)(void*, AsyncClient*, struct pbuf *pb);
typedef void(*AcTimeoutHandler)(void*, AsyncClient*, uint32_t time);
typedef void(*AsNotifyHandler)(void*, size_t event);

enum error_events {
	EE_OK = 0,
	EE_ABORTED,
	        // Callback or foreground aborted connections
	EE_ERROR_CB,
	       // Stack initiated aborts via error Callbacks.
	EE_CONNECTED_CB,
	EE_RECV_CB,
	EE_ACCEPT_CB,
	EE_MAX
...
#endif
	void begin(u8_t backlog = TCP_DEFAULT_LISTEN_BACKLOG);
	void end();
	void setNoDelay(bool nodelay);
	bool getNoDelay();
	uint8_t status();
