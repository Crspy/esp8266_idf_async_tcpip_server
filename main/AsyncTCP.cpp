...
void ACErrorTracker::setCloseError(err_t e) {
	if (e != ERR_OK)
		ESP_LOGD(TAG, "setCloseError() to: %s(%d)\n", _client->errorToString(e), e);
	if (_errored == EE_OK)
		_close_error = e;
}
/**
...
bool AsyncClient::connect(IPAddress ip, uint16_t port, bool secure) {
	...
	bool AsyncClient::connect(const char* host, uint16_t port, bool secure) {
		...
		AsyncClient& AsyncClient::operator=(const AsyncClient& other) {
			if (_pcb) {
#if DEBUG_ESP_ASYNC_TCP
				ESP_LOGD(TAG, "operator=[%u]: Abandoned _pcb(0x%" PRIXPTR ") forced close.\n", getConnectionId(), uintptr_t(_pcb));
#endif
				_close();
			}
		...
		size_t AsyncClient::add(const char* data, size_t size, uint8_t apiflags) {
			...
			if (err != ERR_OK) {
#if DEBUG_ESP_ASYNC_TCP
				ESP_LOGD(TAG, "_add[%u]: tcp_write() returned err: %s(%d)\n", getConnectionId(), errorToString(err), err);
#endif
				return 0;
			}
...
#if DEBUG_ESP_ASYNC_TCP
			ESP_LOGD(TAG, "send[%u]: tcp_output() returned err: %s(%d)", getConnectionId(), errorToString(err), err);
#endif
			_tx_unsent_len = 0;
			return false;
		}
...
#if DEBUG_ESP_ASYNC_TCP
					ESP_LOGD(TAG, "_close[%u]: abort() called for AsyncClient 0x%" PRIXPTR "\n", getConnectionId(), uintptr_t(this));
#endif
					abort();
				}
				_pcb = NULL;
		...
		void AsyncClient::_recv(std::shared_ptr<ACErrorTracker>& errorTracker, tcp_pcb* pcb, pbuf* pb, err_t err) {
			...
			if(NULL == pcb || ERR_OK != err) {
#if DEBUG_ESP_ASYNC_TCP
				ESP_LOGD(TAG, "_recv[%u]:%s err: %s(%d)\n", errorTracker->getConnectionId(), ((NULL == pcb) ? " NULL == pcb!," : ""), errorToString(err), err);
#endif
				assert(ERR_ABRT != err);
			...
			if (pb == NULL) {
#if DEBUG_ESP_ASYNC_TCP
				ESP_LOGD(TAG, "_recv[%u]: pb == NULL! Closing... %d\n", errorTracker->getConnectionId(), err);
#endif
				_close();
				return;
			}
		...
		void AsyncClient::_dns_found(struct ip_addr *ipaddr) {
			...
			void AsyncClient::_s_dns_found(const char *name, ip_addr_t *ipaddr, void *arg) {
				...
				void AsyncServer::begin(u8_t backlog) {
					if (_pcb)
						return;
					...
					if (_connect_cb) {
						...
						if (_ssl_ctx) {
							if (tcp_ssl_has_client() || _pending) {
								...
								if (!new_item) {
									ESP_LOGD(TAG, "### malloc new pending failed!\n");
								...
								if (c) {
									ESP_LOGD(TAG, "_accept[%u]: SSL connected\n", c->getConnectionId());
									...
									ESP_LOGD(TAG, "_accept[_ssl_ctx]: new AsyncClient() failed, connection aborted!\n");
									if (tcp_close(pcb) != ERR_OK) {
										tcp_abort(pcb);
										return ERR_ABRT;
									}
...
#endif
#if DEBUG_ESP_ASYNC_TCP
								ESP_LOGD(TAG, "_accept[%u]: connected\n", errorTracker->getConnectionId());
#endif
								_connect_cb(_connect_cb_arg, c);
								return errorTracker->getCallbackCloseError();
							}
							else {
								ESP_LOGD(TAG, "_accept: new AsyncClient() failed, connection aborted!\n");
								if (tcp_close(pcb) != ERR_OK) {
									tcp_abort(pcb);
									return ERR_ABRT;
								}
				...
				err_t AsyncServer::_poll(tcp_pcb* pcb) {
					if (!tcp_ssl_has_client() && _pending) {
						...
						ESP_LOGD(TAG, "### remove from wait: %d\n", _clients_waiting);
						AsyncClient *c = new(std::nothrow) AsyncClient(pcb, _ssl_ctx);
						if (c) {
							c->onConnect([this](void * arg, AsyncClient *c) {
									_connect_cb(_connect_cb_arg, c);
				...
				err_t AsyncServer::_recv(struct tcp_pcb *pcb, struct pbuf *pb, err_t err) {
					...
					if (!pb) {
						ESP_LOGD(TAG, "### close from wait: %d\n", _clients_waiting);
						p = _pending;
						if (p->pcb == pcb) {
							_pending = _pending->next;
						}
